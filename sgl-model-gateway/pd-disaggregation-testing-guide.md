# SGLang Model Gateway — PD 分离架构测试指南（macOS）

本文档介绍如何在 macOS 上测试 sgl-model-gateway 在 Prefill-Decode (PD) 分离架构下的请求分发能力。
...
手动启动 Gateway 二进制，配合模拟 Worker 来观察真实的 PD 请求分发行为。
...
## 测试方式三：e2e 测试框架（需要 GPU）

项目的 `e2e_test/` 目录提供了完整的端到端测试框架，但需要真实的 SGLang Server 实例（即需要 GPU 环境）。

**在 macOS 上无法直接运行此部分。** 如需运行，请在配备 NVIDIA GPU 的 Linux 服务器上执行。
...
6. **对比 HTTP vs gRPC** → 比较 `src/routers/http/pd_router.rs` 和 `src/routers/grpc/pd_router.rs`
...
MockWorker 模拟了 SGLang Worker 的 HTTP API 接口（`/health`、`/generate`、`/v1/chat/completions` 等），但不执行真正的模型推理。它足以验证 Gateway 的路由逻辑、策略选择、bootstrap 注入、重试容错等功能。
